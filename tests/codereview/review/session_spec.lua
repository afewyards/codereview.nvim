...
describe("review.session", function()
  ...
  describe("publish()", function()
    it("sets published with event and deactivates session", function()
      session.start()
      session.publish("APPROVE")
      local s = session.get()
      assert.is_false(s.active)
      assert.same({ event = "APPROVE" }, s.published)
    end)

    it("clears ai state", function()
      session.start()
      session.ai_start(42)
  ...
  describe("start() clears published state", function()
    it("clears published state", function()
      session.start()
      session.publish("APPROVE")
      session.start()
      local s = session.get()
      assert.is_nil(s.published)
    end)
  end)

  describe("reset() clears published state", function()
    it("clears published state", function()
      session.start()
      session.publish("APPROVE")
      session.reset()
