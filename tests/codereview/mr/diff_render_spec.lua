...
describe("is_outdated", function()
  it("returns true when position.head_sha differs from review.head_sha", function()
    local disc = { notes = { { position = { head_sha = "old" } } } }
    local review = { head_sha = "new" }
    assert.is_true(diff_render.is_outdated(disc, review))
  end)

  it("returns false when position.head_sha matches review.head_sha", function()
    local disc = { notes = { { position = { head_sha = "same" } } } }
    local review = { head_sha = "same" }
...
describe("discussion_line with commit_filter", function()
  ...
  it("returns nil for truly outdated discussion without change_position", function()
    local disc = {
      notes = { { position = { head_sha = "old_sha", new_path = "a.lua", new_line = 42 } } },
    }
    local review = { head_sha = "new_sha" }
    local target_line = diff_render.discussion_line(disc, review)
