...
package.loaded["codereview.config"] = {
  get = function()
    return {
      ai = { enabled = true, provider = "ollama", ollama = { model = "llama3", base_url = "http://localhost:11434" } },
    }
  end,
}
...
describe("ai.providers.ollama", function()
  ...
  it("sends prompt to ollama chat API", function()
    local result, err
    ollama.run("Review this code", function(o, e)
      result = o
      err = e
    end)
    assert.is_nil(err)
  ...
  it("uses custom base_url", function()
    package.loaded["codereview.config"].get = function()
      return {
        ai = { enabled = true, provider = "ollama", ollama = { model = "llama3", base_url = "http://remote:11434" } },
      }
    end
